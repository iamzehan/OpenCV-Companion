...
def Play_Video_from_File():
    ...
    st.markdown("""
                    Playing video from file is the same as capturing it from camera,
    ...
    video_file = st.sidebar.file_uploader("Upload a video to see how it works",
                                          type=["mp4", "avi", "mov"],
...
def Save_Video():
    st.markdown("""
                    ...
                    When working with videos, capturing and processing frames frame-by-frame is a common task. 
                    Unlike images, saving videos involves a bit more effort. 
                    In OpenCV, for images, you can use `cv.imwrite()`.
                    However, for videos, you need to create a `VideoWriter` object.
                    ...
                    - Set the isColor flag. If it's True, the encoder expects color frames; otherwise,
                    it works with grayscale frames.
                    ...
                    - Example codes for different platforms:
                        - In Fedora: DIVX, XVID, MJPG, X264, WMV1, WMV2. (XVID is preferable; 
                        MJPG results in high-size video;
                        X264 gives very small size video)
                        - In Windows: DIVX (More to be tested and added)
                        - In OSX: MJPG (.mp4), DIVX (.avi), X264 (.mkv).
                    - FourCC code can be passed as `cv.VideoWriter_fourcc('M','J','P','G')` or
                    `cv.VideoWriter_fourcc(*'MJPG')` for MJPG.

                    3. Example code:
                    The following code captures from a camera, flips every frame in the vertical direction,
                    and saves the video.
